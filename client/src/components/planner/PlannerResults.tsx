    ...
    try {
      ...
      await submitLead({
        ...
        customFields: [
          ...
          { key: "team_size",          field_value: state.teamSize || "" },
          { key: "workflows_selected", field_value: (state.selectedWorkflows || []).join(", ") },
          { key: "pain_points",        field_value: (state.painPoints || []).join(", ") },
          { key: "tools_used",         field_value: (state.tools || []).join(", ") },
          { key: "human_required",     field_value: (state.humanRequired || []).join(", ") },
          { key: "readiness",          field_value: state.readiness || "" },
          { key: "top_opportunity",    field_value: results?.opportunities?.[0]?.name || "" },
          { key: "est_annual_savings", field_value: `$${annualAdminCost}` },
          { key: "est_hours_saved",    field_value: `${adminHrs} hrs/week` },
          { key: "enquiries_per_week",      field_value: state.volume.enquiriesPerWeek != null ? String(state.volume.enquiriesPerWeek) : "" },
          { key: "appointments_per_week",   field_value: state.volume.appointmentsPerWeek != null ? String(state.volume.appointmentsPerWeek) : "" },
          { key: "active_clients_per_month",field_value: state.volume.activeClientsPerMonth != null ? String(state.volume.activeClientsPerMonth) : "" },
